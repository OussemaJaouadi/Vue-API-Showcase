<script setup lang="ts">
import { computed } from 'vue'

const props = withDefaults(defineProps<{
  color?: string
  backgroundColor?: string
  duration?: number
  blurIntensity?: string
  density?: number
  opacity?: number
}>(), {
  color: 'var(--falling-pattern-color)',
  backgroundColor: 'var(--background)',
  duration: 120,
  blurIntensity: '0.6rem',
  density: 1,
  opacity: 1
})

const patternRows = [
  ['0px', '235px', '150px', '117.5px'],
  ['0px', '252px', '150px', '126px'],
  ['0px', '150px', '150px', '75px'],
  ['0px', '253px', '150px', '126.5px'],
  ['0px', '204px', '150px', '102px'],
  ['0px', '134px', '150px', '67px'],
  ['0px', '179px', '150px', '89.5px'],
  ['0px', '299px', '150px', '149.5px'],
  ['0px', '215px', '150px', '107.5px'],
  ['0px', '281px', '150px', '140.5px'],
  ['0px', '158px', '150px', '79px'],
  ['0px', '210px', '150px', '105px']
]

const backgroundImage = computed(() => patternRows.flatMap(([leftX, y, dotX, dotY]) => [
  `radial-gradient(4px 100px at ${leftX} ${y}, ${props.color}, transparent)`,
  `radial-gradient(4px 100px at 300px ${y}, ${props.color}, transparent)`,
  `radial-gradient(1.5px 1.5px at ${dotX} ${dotY}, ${props.color} 100%, transparent 150%)`
]).join(', '))

const backgroundSize = computed(() => patternRows.flatMap(([, y]) => [
  `300px ${y}`,
  `300px ${y}`,
  `300px ${y}`
]).join(', '))

const startPositions = [
  '0px 220px', '3px 220px', '151.5px 337.5px',
  '25px 24px', '28px 24px', '176.5px 150px',
  '50px 16px', '53px 16px', '201.5px 91px',
  '75px 224px', '78px 224px', '226.5px 230.5px',
  '100px 19px', '103px 19px', '251.5px 121px',
  '125px 120px', '128px 120px', '276.5px 187px',
  '150px 31px', '153px 31px', '301.5px 120.5px',
  '175px 235px', '178px 235px', '326.5px 384.5px',
  '200px 121px', '203px 121px', '351.5px 228.5px',
  '225px 224px', '228px 224px', '376.5px 364.5px',
  '250px 26px', '253px 26px', '401.5px 105px',
  '275px 75px', '278px 75px', '426.5px 180px'
].join(', ')

const endPositions = [
  '0px 6800px', '3px 6800px', '151.5px 6917.5px',
  '25px 13632px', '28px 13632px', '176.5px 13758px',
  '50px 5416px', '53px 5416px', '201.5px 5491px',
  '75px 17175px', '78px 17175px', '226.5px 17301.5px',
  '100px 5119px', '103px 5119px', '251.5px 5221px',
  '125px 8428px', '128px 8428px', '276.5px 8495px',
  '150px 9876px', '153px 9876px', '301.5px 9965.5px',
  '175px 13391px', '178px 13391px', '326.5px 13540.5px',
  '200px 14741px', '203px 14741px', '351.5px 14848.5px',
  '225px 18770px', '228px 18770px', '376.5px 18910.5px',
  '250px 5082px', '253px 5082px', '401.5px 5161px',
  '275px 6375px', '278px 6375px', '426.5px 6480px'
].join(', ')

const rootStyle = computed(() => ({
  '--falling-color': props.color,
  '--falling-background': props.backgroundColor,
  '--falling-duration': `${props.duration}s`,
  '--falling-blur': props.blurIntensity,
  '--falling-density': `${8 * props.density}px`,
  '--falling-opacity': props.opacity,
  '--falling-start': startPositions,
  '--falling-end': endPositions
}))
</script>

<template>
  <div class="falling-pattern pointer-events-none" :style="rootStyle">
    <div
      class="falling-pattern__plane"
      :style="{
        backgroundColor,
        backgroundImage,
        backgroundSize
      }"
    />
    <div class="falling-pattern__veil" />
  </div>
</template>
